...
function renderRivalDetail(rivalId) {
    var rival = rivals.find(function (r) { return r.id === rivalId; });
    if (!rival) return;
    ...
    html += '<div class = "player-card">'
        ...
        + '<span id="player-name-display" style="font-size:20px;font-weight:700;color:var(--text);">' + rival.playerName + '</span>'
        + '</div>'
        + '<div class ="player-card-stats">'
        + '勝率 ' + (rival.total > 0 ? Math.round(rival.wins / rival.total * 100) : 0) + '%(' + rival.wins + '勝 / ' + rival.total + '試合)'
        + '</div>';
    rival.decks.forEach(function (d) {
        var parts = [d.b, d.r, d.bt, d.c].filter(Boolean).join(' / ');
        html += '<div class="player-card-deck">'
            + '<div class="player-card-deck-name">' + (d.name || 'デッキ') + '</div>'
            + '<div>' + parts + '</div>'
            + '</div>';
    ...
    html += '<div style="padding:0 16px;margin-top:16px;">'
        + '<button class="btn-sm danger" onclick="deleteRival(' + rival.id + ')" '
...
function renderQRReader() {
    ...
    // 状態テキスト
    html += '<div id="qr-status" style="text-align:center;font-size:13px;color:var(--text2);padding:8px 16px;">'
        + 'カメラを起動中...</div>';
...
function startQRCamera() {
    ...
    navigator.mediaDevices.getUserMedia({
        ...
        if (status) status.textContent = 'カメラの起動に失敗しました：' + err.message;
    });
}
...
function renderRivalPreview(data) {
    ...
    html += '<div style="padding:12px 16px 8px;font-size:13px;color:var(--text2);">読み取ったプレイヤーカード</div>';

    // プレイヤーカード表示
    html += '<div class="player-card">'
        + '<div class="player-card-name">' + data.n + '</div>'
        + '<div class="player-card-stats">勝率 '
        + (data.t > 0 ? Math.round(data.w / data.t * 100) : 0)
        + '% (' + data.w + '勝 / ' + data.t + '試合)</div>';
    ...
    // 追加ボタン
    html += '<div style="padding:0 16px;margin-top:16px;display:flex;gap:8px;">'
